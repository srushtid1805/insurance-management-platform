  ...
  try {
    ...
    if (req.user.role === "agent") {
      const belongsToAgent =
        await checkCustomerBelongsToAgent(
          user_id,
          req.user.id
        );
      ...
      if (!belongsToAgent) {
        return res.status(403).json({
          message:
            "You can assign policies only to your own customers"
        });
      }
    }
...
const fetchUserPolicyById = async (req, res) => {
  try {
    ...
    const userPolicy = await getUserPolicyById(
      id,
      req.user.role,
      req.user.id
    );
    ...
    if (!userPolicy) {
      return res.status(404).json({
        message:
          "User policy not found or you cannot access it"
      });
    }
...
const updateUserPolicyDetails = async (
  req,
  res
) => {
  try {
